...
DescriptiveTab <- function(dataframe, num, char, grp, title="Table", label.option=F, option=F, digit=1, num.test="Wilcoxon/Kruskal"){
  #Generates descritive statistics for data 
  #Developped by: Tianqi Chen
  #Date: 10/26/2016
  #
  #Args:
  # dataframe: data to be used
  # num:       names of numeric variables in vector median(min-max)
  # char:      names of categorical variables in vector
  # grp:       group names in vector
  # title:     title of the table
  # label.option: T = use variable label rather than variable name
  # option:    T = min and max for numeric variable and percentage for categorical variable
  # digit:     round the numeric statistics
  # num.test:  a character string specifying the test method for numeric variable among groups, must be one of "None", "ANOVA", "Wilcoxon/Kruskal".
  #            "Wilcoxon/Kruskal" means if the group variable has two groups, then use Wilcoxon sum rank test, if has more than two groups, use 
  #            Kruskal-Wallis test.
  #
  #Returns:
  ...
  # Wilcoxon test for non-parametric variable and two groups; Kruiskal test for more than two groups (excluding missing group level)
  # Chi-square test for categorical variables (excluding missing levels, both variable and group)
